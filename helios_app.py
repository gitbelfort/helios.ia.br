...
# ==============================================================================
# HELIOS v9.7 CORE (API KEY + SHIELD)
# ==============================================================================

# 1. Usando os modelos oficiais da API Key (AI Studio)
MODELO_TEXTO_FIXO = "gemini-2.0-flash" 
MODELO_IMAGEM_FIXO = "gemini-3-pro-image-preview"

# 2. Instanciando o Cliente via API KEY simples
try:
    client = genai.Client(
    ...
    st.error(f"Erro ao inicializar API: {e}")
...
def generate_image_pixels(prompt_text, aspect_ratio, reference_image=None):
    ...
    try:
        ...
        st.error(f"Erro no Motor Visual API: {e}")
        return None
...
# UI PRINCIPAL
# ==============================================================================
st.title("🟡 HELIOS // STUDIO v9.7")

st.markdown("""<div class="instruction-box"><strong>MANUAL v9.7:</strong> Sistema conectado via API Key com Escudo de Retentativa Ativo.</div>""", unsafe_allow_html=True)
...
with col1:
    ...
    if uploaded_file:
        ...
        if current_id != st.session_state.last_uploaded_file_id:
            with st.spinner("PROCESSANDO E VERIFICANDO..."):
    ...
    with b_col1:
        if st.button("GERAR IMAGEM", type="primary", use_container_width=True, disabled=not st.session_state.security_check_passed, key=f"gen_{reset_k}"):
            with st.spinner("RENDERIZANDO (Pode levar alguns segundos com o Escudo ativo)..."):
...
st.markdown("""<div class="footer">CONEXÃO ESTABELECIDA: API KEY | SHIELD ATIVO | HELIOS.IA.BR</div>""", unsafe_allow_html=True)
